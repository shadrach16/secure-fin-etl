...
from FILETEST.config import LOG_PATH,FULL_LOG_PATH,SQLITE_DIRS
from FILETEST.ricaLicense import genLicense
...
def get_from_account(execute, accountId, key):
    ...
    account = execute(f""" 
         select * from rica_account
                           Where RICAFILETESTNO = '{accountId}'
    """)
...
def find_grouping(v_groupBy):
    ...
    elif v_groupBy.upper() == 'FILETESTOFFICER':
    ...
    elif v_groupBy.upper() == 'FILETEST-TYPE':
        DISTINCT_FIELD = 'trans_type'
