...
import smtplib
from ACCOUNT.func import logError
...
template_path = os.path.join(get_sqlite_con_dir(),"bots", "PAYLOAD_DB_MIGRATIONS", "ACCOUNT","templates")
images_path = os.path.join(get_sqlite_con_dir(),"bots", "PAYLOAD_DB_MIGRATIONS","ACCOUNT","images")
date = now.strftime('%d-%b-%y %H:%M:%S %p').upper()
...
def send_response_mail(options={}, spf={} ):
    ...
    subject = f"""{spf.get("ricaAppsId")} - [URGENT] Error in Payload: ACCOUNT"""
...
def custom_send(subject, html_content, recipient_list=[],  spf={}):
    ...
    try:
        ...
        logError("OutMail").log(log_args,'ACCOUNT')
        print('mail sent')
