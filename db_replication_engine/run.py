...
from ACCOUNT.func import get_env_settings,get_ip,logError,replaceParams
from ACCOUNT.func import convert_time, format_int, format_int_bin, is_time_between, get_first, get_last, clean,GenRunDate,get_query_fields,get_multi_value,get_spf,get_footer_label,clean_emails,find_grouping,license_expired,get_expired_notification
from ACCOUNT.datahelper import getChart3Data,getLineChartData,generateAnalytics
from ACCOUNT.date import date_func, create_date, create_time
from ACCOUNT.ricaED import E 

from ACCOUNT.mailer import send_response_mail
...
from ACCOUNT.xlsx import create_lite_excel
from ACCOUNT.first import  step_1, get_status
from ACCOUNT.config import LOG_PATH,FULL_LOG_PATH,STATUS,MAP, time_formatter,date_formatter,DTTYPES,language
from ACCOUNT.common import load_queries
import re
...
class DatabaseConnector():
	...
	def get(self,service):
		...
		sql_query = replaceParams( service.get('ricaQueryPanel'),query_params)
		print(sql_query)
		...
		df = pd.DataFrame(data)
		map_fields = service.get('FIELD_MAPPER') or {} 
		df.fillna('', inplace=True)
	...
	def post(self,data=[],table_name="",pk="",columns_dtypes={},file_name="",service={},batch_size=1000):
		...
		try:
			...
			log_args = {
				"ricaLogId":f'Table-{table_name}-{date_now}-{current_time}',
				'ricaApplication':"ETL Service: ACCOUNT",  
				'Folder':table_name,
				'ricaTable':table_name,
				'ricaText':f"{e}",
				'ricaStatus':"Failed",
...
def my_transform(chunk: Iterator[dict],service={}) -> Iterator[dict]:
	df = pd.DataFrame(chunk)  # Assuming `chunk` is an iterator of dictionaries
	df.fillna('', inplace=True)
	...
	for record in renamed_df[filtered_mapper.values()].to_dict('records'):
		yield record


table_exist = None

def post_data(data, connector,configs_vars):
...
def run_operation(service):
	operation = service.get('OPERATIONS')
	print("Running Service: ",service['ricaPayloadId'],str(operation))

	src_conn = connect_to_database(service['ricaConnector'])
	# dest_conn = connect_to_database(service['ricaDestinationConnector'])

	SELECT_SQL =   replaceParams( service.get('ricaQueryPanel'),service)
	# print('SELECT_SQL',"\n",dedent(SELECT_SQL))
	map_fields = service.get('FIELD_MAPPER')
	data = etl.fetchall(SELECT_SQL,src_conn,
		row_factory=dict_row_factory,
		transform=lambda chunk: my_transform(chunk, service))
	
	print('len_data pulled',len(data))

	post_data(data, service['ricaDestinationConnector'],service) 
...
class PayloadService():
	...
	def run_service(self,scenarioId,journalEntriesData=None,cursor=None,scenarioRecord=None):
		...
		z=[]
		fields = ["RICATRANSREFID","RICATRANSID",'RICANARRATIVE','RICATRANSTYPE',"RICAACCOUNTID","RICAACCOUNTricaPayloadId","RICATRANSCODE","RICALCYAMOUNT","RICAENTRYDATE","RICAENTRYTIME"]
		...
		if still_valid:
			...
			if scenarioRecord and ricaRunStatus and int(ricaRunStatus["ricaModelflag"]) and scenarioRecord["ricaQueryPanel"]:
				...
				# print('scenarioRecord2',scenarioRecord)
